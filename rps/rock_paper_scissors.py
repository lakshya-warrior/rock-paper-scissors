...
def draw_screen(window, w_width, w_height):
    ...
    t1 = Label(window, text='Rock Paper Scissors', font=('Times New Roman', 45, 'bold'), fg='#00008B',
               background="#ADD8E6")
    t1.pack()

    you_label = Label(window, text='You', font=('Times New Roman', 45, 'bold'), fg='#FFFDD0', background="#ADD8E6")
    ...
    comp_label = Label(window, text='Computer', font=('Times New Roman', 45, 'bold'), fg='#FFFDD0',
                       background="#ADD8E6")
    comp_label.place(x=w_width / 2 - 500, y=110)

    x_btn = w_width / 2 - 120

    r_btn = Button(screen, text="Rock", font=('Times New Roman', 20, 'bold'), fg='#FFD700', bg='#4169E1', width=15,
                   height=2, command=lambda: show_image("rock", ""))
    r_btn.place(x=x_btn - 300, y=w_height - 200)

    p_btn = Button(screen, text="Paper", font=('Times New Roman', 20, 'bold'), fg='#FFD700', bg='#4169E1', width=15,
                   height=2, command=lambda: show_image("paper", ""))
    p_btn.place(x=x_btn, y=w_height - 200)

    s_btn = Button(screen, text="Scissors", font=('Times New Roman', 20, 'bold'), fg='#FFD700', bg='#4169E1', width=15,
                   height=2, command=lambda: show_image("scissors", ""))
    s_btn.place(x=x_btn + 300, y=w_height - 200)

    reset = Button(screen, text="Reset", font=('Times New Roman', 15, 'bold'), fg='#FFD700',
...
comments_text_list = ["TIE!", "You LOST!", "You WON!"]
# Images
for i in range(0, 6):
...
# Comments
for i in range(0, 3):
    score_label.append(Label(screen, text=comments_text_list[i], font=('Times New Roman', 30, 'bold'), fg='green',
                             background="#ADD8E6"))
